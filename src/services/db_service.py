...
def save_daily_log(symbol, data):
    ...
    ''', (entry_id, today, symbol, round(data.get('prev_close', 0), 2), round(data.get('current', 0), 2),
          round(data.get('open', 0), 2), data.get('bias', 'Neutral'), round(data.get('downside', 0), 2),
          round(data.get('upside', 0), 2), round(data.get('t_impact', 0), 2), round(data.get('s_impact', 0), 2),
          round(data.get('vol_ratio', 1.0), 2), round(data.get('london_high', 0), 2), round(data.get('london_low', 0), 2)))
    conn.commit()
    conn.close()

def get_history_with_validation():
    conn = sqlite3.connect('market.db')
    df = pd.read_sql_query("SELECT * FROM daily_forecasts ORDER BY date DESC LIMIT 15", conn)
    conn.close()
    return df